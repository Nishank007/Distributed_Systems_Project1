...
def add(args, conn):       # Responds to the client's request to rename a file
    ...
    if args:
        ...
        if len(args_array) == 2:
            ...
            result = str(_result)
            results['result'] = result  # Store result in the table/dictionary
            do_other_stuff()
...
def calculate_pi(conn):
    ...
    results['result'] = str(_calculate_pi())
    do_other_stuff()
...
def send_ack_to_client(conn):
    """ Sends acknowledgement to client
        Proceeds to do other stuff before computing the result
    """
...
def main():     # The entry point for the server
    ...
    while True:
        ...
        if request:
            ...
            (command, arguments) = parse_reequest(request)
        ...
        # Some empty lines for formating server output
        print()
        print()
    server.close()
